            ...
            <tbody>
                @foreach ($standard_recipe as $data)
                    <tr>
                        ...
                        <td>{{ $data->unit2 }}</td>
                        <td>{{ $data->p_waste }}</td>
                        <td>{{ $data->qty_waste }}</td>
                        <td>{{ $data->p_use }}</td>
                        <td>{{ $data->qty_use }}</td>
                        <td>Rp. {{ number_format($data->conv, 2, ',', '.') }}</td>
    ...
    $.ajax({
        ...
        $(document).on('click', '.btn-danger', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('standard-recipe.clear') }}",
                        type: 'DELETE',
                        data: {
        ...
        $('#barangTable tbody').on('click', '.remove-row', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/standard-recipe/${rowId}`, // Adjust your route accordingly
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
                        success: function(response) {
                            ...
                            'standardRecipeRows'); // or update accordingly
