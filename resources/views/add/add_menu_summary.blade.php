            ...
            <div style="width: 100%">
                ...
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Menu Summary Detail</h3>
                    </div>
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form method="POST" action="{{ route('menu_summary.store') }}">
                        @csrf
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="menu" id="exampleInputMenu">
                                    ...
                                    @foreach ($menu_pricing as $data)
                                        <option value="{{ $data['menu_id'] }}" data-total="{{ $data['total_cost'] }}">
                                            {{ $data['menu_name'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label for="total_bahan_cost">Total Bahan Cost</label>
                                <input type="number" class="form-control" id="total_bahan_cost" name="total_bahan_cost"
                                    placeholder="Pick menu first" readonly>
                            </div>

                            <div class="form-group">
                                <label for="final_price">Final Price (+5%)</label>
                                <input type="text" class="form-control" id="final_price" name="final_price" placeholder="Enter menu first" readonly>
                            </div>
...
<script>
    ...
    $('#exampleInputMenu').on('change', function() {
        const selected = $(this).find('option:selected');  // get selected option
        const total = parseFloat(selected.data('total') || 0);
        const final = (total * 1.05).toFixed(2); // Add 5%

        $('#total_bahan_cost').val(total);
