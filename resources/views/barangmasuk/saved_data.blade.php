...
<script>
    ...
    $.ajax({
        ...
        $('#barangTable tbody').on('click', '.remove-row', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/barang-masuk/${rowId}`, // Adjust your route accordingly
                        type: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
                        success: function(response) {
                            ...
                            'barangMasukRows'); // or update accordingly
    ...
    $(document).ready(function() {
        $('#printLaporanBtn').on('click', function() {
            ...
            // Download CSV file
            downloadCSV("laporan_barang_masuk.csv", csv);
        });
    });
