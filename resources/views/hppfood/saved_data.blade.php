        ...
        <tbody>
            @foreach ($hpp_foods as $data)
                <tr>
                    ...
                    <td>{{ $data->kategori }}</td>
                    <td>{{ $data->menu }}</td>
                    <td>Rp. {{ number_format($data->hpp, 2, ',', '.') }}</td>
                    <td>Rp. {{ number_format($data->hjp, 2, ',', '.') }}</td>
                    <td>Rp. {{ number_format($data->hjp_nett, 2, ',', '.') }}</td>
                    <td>{{ $data->percent_cost }}%</td>
                    <td>{{ $data->created_at }}</td>
                    <td><a href="#" class="text-danger remove-row" data-id="{{ $data->id }}"><i class="fas fa-trash-alt"></i></a></td>
                </tr>
            @endforeach
        </tbody>
...
<script>
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            // Clear localStorage
                            localStorage.removeItem('hppFoodRows');
    ...
    $(document).ready(function() {
        $('#printLaporanBtn').on('click', function() {
            ...
            // Download CSV file
            downloadCSV("laporan_konversi_harga_bahan.csv", csv);
        });
    });
